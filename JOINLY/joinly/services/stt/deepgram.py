...
class DeepgramSTT(STT):
    ...
    def __init__(  # noqa: PLR0913
        ...
        *,
        ...
        """
        self._client_config = DeepgramClientOptions(
            options={"keepalive": "true", "keep_alive": True}
        )
    ...
    def _create_client(self) -> AsyncListenWebSocketClient:
        """Create a fresh Deepgram listen websocket client."""
        dg = DeepgramClient(config=self._client_config)
        return dg.listen.asyncwebsocket.v("1")  # type: ignore[attr-type]

    def _register_handlers(self) -> None:
        """Register Deepgram websocket event handlers once per client."""
    ...
    async def _reset_client(self) -> None:
        """Replace the websocket client after a failed or closed connection."""
        ...
        for attempt in range(1, self._connect_retries + 1):
            try:
                ...
                if attempt >= self._connect_retries:
                    logger.exception(
                        "Deepgram STT websocket start failed after %d attempt(s).",
                        attempt,
                    )
                    raise
                logger.warning(
                    "Deepgram STT websocket start failed on attempt %d/%d; retrying.",
                    attempt,
                ...
                if attempt >= self._connect_retries:
                    break
                logger.warning(
                    "Deepgram STT websocket was not connected after attempt %d/%d; "
                    "retrying.",
                    attempt,
                    self._connect_retries,
                )
    ...
    async def _ensure_connected(self) -> None:
        """Ensure the live Deepgram websocket is connected for the next utterance."""
        if self._queue is None:
