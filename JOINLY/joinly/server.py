...
# Set by session_lifespan; lets tests inspect the live page for screenshots.
_active_meeting_session: Any = None

# Set by session_lifespan; lets tests/tools inspect the live page.
_active_meeting_session: Any = None
...
async def session_lifespan(server: FastMCP) -> AsyncIterator[SessionContext]:
    ...
    try:
        import joinly.server as _smod  # noqa: PLC0415
        _smod._active_meeting_session = meeting_session
...
async def get_chat_history(
    ...
    """Get the chat history from the current meeting session only.

    Filters to messages sent AFTER the bot joined this meeting, so the LLM
    never sees old summaries or messages from previous sessions.
    Falls back to the last 30 recent messages if timestamps cannot be compared.
    """
    from datetime import datetime, timezone  # noqa: PLC0415

    ms: MeetingSession = ctx.request_context.lifespan_context.meeting_session
    history = await ms.get_chat_history()
    join_time = ms.join_time  # UTC datetime when bot joined, or None

    current_session: list = []
    for m in history.messages:
        # Skip system rows (no sender = "Meeting ended", "Meeting started", etc.)
        if not m.sender or not m.timestamp:
            continue
        # If we know when this session started, keep only messages after that.
        if join_time is not None:
            try:
                msg_dt = datetime.fromisoformat(m.timestamp.replace("Z", "+00:00"))
                if msg_dt < join_time:
                    continue  # Message predates this session
            except (ValueError, AttributeError):
                pass  # Unparseable timestamp — include it to be safe
        current_session.append(m)

    # Cap at 50 messages to avoid token bloat even within a single session.
    return MeetingChatHistory(messages=current_session[-50:])
...
@mcp.tool(
    ...
    description=(
        ...
        "Call once after joining so the panel is ready before sending messages."
    ),
)
async def open_chat_panel(
    ctx: Context,
) -> str:
    """Open the in-meeting chat panel so messages can be sent immediately."""
    ms: MeetingSession = ctx.request_context.lifespan_context.meeting_session
    await ms.open_chat_panel()
    return "Chat panel is open."
