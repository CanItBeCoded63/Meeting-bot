...
_NUM_BANDS = 7
_LOGO_PATH_ENV = "JOINLY_CAMERA_LOGO_PATH"
...
<svg viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg">
    <defs>
        <linearGradient id="alexBg" x1="88" y1="72" x2="424" y2="440"
            gradientUnits="userSpaceOnUse">
            <stop offset="0" stop-color="#13C8B5"/>
            <stop offset="0.48" stop-color="#236DFF"/>
            <stop offset="1" stop-color="#101832"/>
        </linearGradient>
        <filter id="softShadow" x="-20%" y="-20%" width="140%" height="140%">
            <feDropShadow dx="0" dy="14" stdDeviation="16"
                flood-color="#071023" flood-opacity="0.36"/>
        </filter>
    </defs>
    <rect x="36" y="36" width="440" height="440" rx="112" fill="url(#alexBg)"/>
    <rect x="74" y="156" width="364" height="178" rx="52"
        fill="rgba(255,255,255,0.16)"/>
    <text x="256" y="286" text-anchor="middle" filter="url(#softShadow)"
        font-family="Verdana, Arial, sans-serif" font-size="94" font-weight="800"
        letter-spacing="2" fill="#FFFFFF">ALEX</text>
    <circle cx="148" cy="386" r="14" fill="#FFFFFF" opacity="0.74"/>
    <circle cx="190" cy="386" r="14" fill="#FFFFFF" opacity="0.42"/>
    <circle cx="232" cy="386" r="14" fill="#FFFFFF" opacity="0.74"/>
    <circle cx="274" cy="386" r="14" fill="#FFFFFF" opacity="0.42"/>
    <circle cx="316" cy="386" r="14" fill="#FFFFFF" opacity="0.74"/>
    <circle cx="358" cy="386" r="14" fill="#FFFFFF" opacity="0.42"/>
</svg>
""".strip()


def _svg_to_data_uri(svg: str) -> str:
    """Encode an SVG string as a browser-safe image data URI."""
    return "data:image/svg+xml," + quote(svg, safe="")


_DEFAULT_LOGO_SRC = _svg_to_data_uri(_ALEX_LOGO_SVG)


def _resolve_logo_src(logo_path: str | None = None) -> str:
    """Return the configured camera logo as a data URI."""
    raw_logo_path = (logo_path or os.getenv(_LOGO_PATH_ENV) or "").strip()
    if not raw_logo_path:
        return _DEFAULT_LOGO_SRC

    path = Path(raw_logo_path)
    if not path.is_file():
        logger.warning(
            "Camera logo file not found: %s. Using the default Alex logo.",
            path,
        )
        return _DEFAULT_LOGO_SRC
...
class CameraFeed:
    ...
    def __init__(self, writer: AudioWriter, logo_path: str | None = None) -> None:
    ...
    async def install(self, meeting_page: Page) -> None:
        ...
        script = _CAMERA_OVERRIDE_TEMPLATE.format(
            ...
            h=_CAM_HEIGHT,
            n_bands=_NUM_BANDS,
            logo_src=json.dumps(_resolve_logo_src(self._logo_path)),
