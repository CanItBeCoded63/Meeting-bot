...
class GoogleMeetBrowserPlatformController(BaseBrowserPlatformController):
    ...
    async def get_participants(self, page: Page) -> list[MeetingParticipant]:
        ...
        for item in await participants_list.locator("div[role='listitem']").all():
            participant = await self._participant_from_list_item(item)
    ...
    def _participants_list(self, page: Page) -> Locator:
        """Return the currently supported Google Meet participants list locator."""
        return page.locator(
    ...
    async def _visible_participants_button(self, page: Page) -> Locator | None:
        """Return a visible Google Meet people/participants button, if present."""
        candidates = [
            page.get_by_role(
                "button",
                name=re.compile(
                    r"people|participants|show everyone",
                    re.IGNORECASE,
                ),
            ).first,
            page.locator(
    ...
    async def _participant_from_list_item(
        self,
        item: Locator,
    ) -> MeetingParticipant | None:
        """Parse a Google Meet participants-list item."""
        raw_name = await item.get_attribute("aria-label")
        name = raw_name.split(",", 1)[0].strip() if raw_name else None
        if not name:
            return None

        infos = []
        if (
            (raw_name and "you" in raw_name.casefold())
            or await item.locator('span:has-text("(You)")').count() > 0
        ):
            infos.append("You")
