...
class ZoomBrowserPlatformController(BaseBrowserPlatformController):
    ...
    async def send_chat_message(self, page: Page, message: str) -> None:
        ...
        await self._select_room_chat_recipient(page)
    ...
    async def _select_room_chat_recipient(self, page: Page) -> None:
        """Prefer Zoom's room-wide chat recipient before sending a message."""
        room_pattern = re.compile(r"everyone|meeting group chat", re.IGNORECASE)
        recipient_pattern = re.compile(
            r"to:|recipient|direct message|everyone|meeting group chat",
            re.IGNORECASE,
        )
        recipient_buttons = [
            page.get_by_role("button", name=recipient_pattern).first,
            page.locator('[role="button"][aria-label*="recipient" i]').first,
            page.locator('[role="button"][aria-label*="direct message" i]').first,
            page.locator('button:has-text("Everyone")').first,
        ]

        for button in recipient_buttons:
            ...
            if room_pattern.search(button_text):
                return
            ...
            room_option = page.locator(
                ...
                '[role="menuitem"]:has-text("Everyone"), '
                'li:has-text("Everyone"), '
                'div:has-text("Meeting group chat")'
            ).first
            if await room_option.is_visible():
                await room_option.click(timeout=1000)
                await page.wait_for_timeout(200)
                return

        logger.warning(
            "Zoom room chat recipient could not be confirmed; "
            "sending with current recipient."
        )
