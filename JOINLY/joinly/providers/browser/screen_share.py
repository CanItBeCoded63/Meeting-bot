...
({ w, h }) => {
    ...
    // --- getDisplayMedia override (reinstall if not already patched via init_script) ---
    // Match the working azure/main path: let Chromium perform real screen capture
    // while forcing no audio and hinting browser/self capture. Browser flags auto-
    // select the Xvfb desktop so participants see the canvas overlay.
    if (!navigator.mediaDevices.__joinlyPatched) {
        ...
        navigator.mediaDevices.getDisplayMedia = async function(constraints) {
            ...
            try {
                ...
                window.__scShareErr = 'native-gdm: ' + String(e);
