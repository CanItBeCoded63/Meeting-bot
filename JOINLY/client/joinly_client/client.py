...
class JoinlyClient:
    ...
    def __init__(
        ...
        *,
        ...
        self._sent_chat_messages: deque[str] = deque(maxlen=50)
    ...
    async def _utterance_update(self) -> None:
        ...
        if new_transcript.segments:
            latest = new_transcript.segments[-1]
            logger.debug(
                "Utterance update: %d new segment(s) since last trigger. "
                "Latest — %s: '%s'",
                len(new_transcript.segments),
                latest.speaker or "unknown",
                latest.text,
            )
        import time
    ...
    async def open_chat_panel(self) -> None:
        ...
        Raises RuntimeError (wrapped in ToolError) when the panel cannot be
        opened within the server-side timeout.
        """
        if not self.joined:
            msg = "Not joined to a meeting"
            raise RuntimeError(msg)

        await self.client.call_tool("open_chat_panel")
    ...
    async def send_chat_message(self, message: str) -> None:
        ...
        self._sent_chat_messages.append(self._normalize_chat_message(message))
        import time
        self._last_trigger_time = time.time()

    def is_sent_chat_message(self, text: str) -> bool:
        """Return whether text likely matches a chat message sent by this client."""
        normalized = self._normalize_chat_message(text)
        if not normalized:
            return False
        return any(
            sent == normalized or sent in normalized or normalized in sent
            for sent in self._sent_chat_messages
        )
