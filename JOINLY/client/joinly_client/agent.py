    ...
    def _sanitize_messages_for_llm(  # noqa: C901, PLR0912, PLR0915
        ...
        for message in messages:
            ...
            if isinstance(message, ModelResponse):
                ...
                if not has_text:
                    # Covers both tool-call-only responses and genuinely empty
                    # ones (zero parts) — the latter otherwise serializes to
                    # content=null with no tool_calls, which some backends
                    # (e.g. gpt-5.2-chat) reject outright on the next call.
