...
class BotSession:
    ...
    async def start(
        ...
        if is_deepgram:
            ...
            from src.teams_agent.pipeline_sarvam import create_and_run_pipeline

            logger.info("Starting Sarvam pipeline...")
            self._pipeline_task = asyncio.create_task(
                create_and_run_pipeline(
                    self.shutdown_event,
                    ws_ready_event,
